...
type LoadState = { status: "loading" } | { status: "error"; message: string } | { status: "ready"; invitation: InvitationDetails };
...
export function InvitationPage() {
  ...
  return (
    <Box sx={{ minHeight: "100vh", display: "grid", placeItems: "center", p: 3 }}>
      <Paper sx={{ width: "100%", maxWidth: 560, p: 4, border: "1px solid rgba(255,255,255,0.08)" }} data-testid="invitation-card">
        <Stack spacing={3}>
          ...
          {state.status === "ready" ? (
            <>
              <Typography>
                You have been invited to join <strong data-testid="invitation-organization">{state.invitation.organizationName}</strong> as{" "}
              ...
              {state.invitation.status !== "pending" ? (
                <Alert severity="warning">This invitation is {state.invitation.status} and can no longer be accepted.</Alert>
              ) : null}
              {emailMismatch ? (
