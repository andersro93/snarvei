      ...
      <DialogContent>
        <Stack spacing={2} sx={{ pt: 1 }}>
            ...
            <List dense disablePadding data-testid="team-members-list">
              {teamMembers.map((teamMember) => {
                ...
                return (
                  ...
                  >
                    <ListItemText primary={member?.user.name ?? teamMember.userId} secondary={member?.user.email ?? null} />
                  </ListItem>
                );
              })}
            </List>
          ...
          <Stack direction="row" spacing={1} sx={{ alignItems: "center" }}>
              ...
              >
                {candidates.length ? (
                  candidates.map((member) => (
                    <MenuItem key={member.user.id} value={member.user.id} data-testid={`add-team-member-option-${member.user.email}`}>
            ...
            <Button variant="contained" disabled={busy || !selectedUserId} onClick={() => void add()} data-testid="add-team-member-button">
              Add
            </Button>
          </Stack>
        </Stack>
