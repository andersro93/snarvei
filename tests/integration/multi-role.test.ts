...
describe("members with multiple roles", () => {
  it("a member whose role string also contains admin gets admin-level access", async () => {
    ...
    const invite = await request(`${ORIGIN}/api/auth/organization/invite-member`, jsonInit("POST", { email: inviteeEmail, role: "member", organizationId: organization.id }, owner));
    expect(invite.status).toBe(200);
    const invitation = (await invite.json()) as { id: string };
    const invitee = await signUp({ email: inviteeEmail });
    const accept = await request(`${ORIGIN}/api/auth/organization/accept-invitation`, jsonInit("POST", { invitationId: invitation.id }, invitee));
    ...
    await env.DB.prepare("UPDATE members SET role = ? WHERE organization_id = ? AND user_id = ?").bind("member,admin", organization.id, invitee.userId).run();

    expect((await request(`${ORIGIN}/api/links/${link.id}`, { headers: { cookie: invitee.cookie } })).status).toBe(200);
    const orgLinks = (await (await request(`${ORIGIN}/api/organizations/${organization.id}/links`, { headers: { cookie: invitee.cookie } })).json()) as Array<{ id: string }>;
